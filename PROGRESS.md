...
**Date:** 2026-06-11
**Status:** ✅ M6–M9 COMPLETE (2026-06-11). The full `super_plan_2026-06-11.md` ran end-to-end via subagent execution. The TUKU pilot is now an honest single-well sequential estimator (predict→reveal→adjust), leak-proof via a `TimeOracle`, graded against the 264 GENUINE field visits, with split-conformal uncertainty and a cadence-degradation curve. The portfolio GPS-only survey ran at 29 of 37 stations.

> **GATE M8 — DP-SEQ = PARTIAL.** ACCURACY criterion PASSED: all 6 TUKU layers meet Level-1 thresholds at annual cadence or sparser, both in the blind era (2019–2023) and the held-out 2024 confirmatory year (`tau_demo_TUKU/results/seq/cadence_degradation_curve.json`, `confirmatory_2024.json`). Minimum cadence: F1/T1/T2 need NO visits (GPS carrier alone); F2/F3/F4 need ≥ annual visits. Annual visits recover 78–85% of skill for the deep layers vs the no-visit baseline; monthly adds only +3–4 pp. COVERAGE criterion UNDETERMINED: conformal band coverage is sample-limited (computed on 2–6 points at sparse cadence; `MIN_SAMPLES`=20 not reached in long-horizon buckets given ~15 yr of data) — a documented limitation, not a model failure. Full narrative: `discussions/SEQ_REHEARSAL_FINDINGS_20260611.md`.
>
...
**Next (BLOCKED, awaiting human review of M8+M9):** ratify DP-SCOPE (single-well pivot) and DP-SEQ; decide whether to (a) extend the conformal calibration record for a definitive coverage guarantee, (b) augment the failing thick-layer stations with per-layer GWL head (the M8 protocol applied portfolio-wide), and (c) open Part 2 (per-station physics) / Part 3 (8,577 grid points). Structural finding to weigh: with the GPS carrier term present, 4 of 6 layers calibrate to purely elastic head response (carrier absorbs the inelastic trend); only T2/F3 keep an inelastic term, and F3's true consolidation lag (~815 days) exceeds the τ=120 production cap.
