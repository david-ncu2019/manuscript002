...
> - **ACCURACY: PASS but weak-bar.** Layers meet Level-1 at annual cadence largely because the periodic reveals fix the datum; with zero visits F1/T1/T2 already pass, so the threshold (not dynamics) carries those layers.
> - **SKILL: RETRACTED → corrected (F-1).** The earlier 0.79/0.82 annual skill was measured against a contaminated no-visit baseline (model entered the blind era −19.3 mm / −44.3 mm off-datum from the unanchored seed walk). Against a fair *anchor-once* baseline (script 27, `honest_skill_table.json`), honest annual skill is **≤ 0 for F2/T2** (no dynamic skill beyond the one-time datum fix), **+0.19 for F3**; only monthly cadence buys real dynamics (F3 +0.56, F2 +0.23). F4 is *hurt* by monthly visits (−0.20).
> - **COVERAGE: RETRACTED → FAIL (F-2).** Not "undetermined": at semiannual there are 27 band points/layer (≥ the 20-sample minimum) and only **3/6 layers reach 0.85 → FAIL** (script 29, `coverage_reckoning.json`). PASS only at monthly (5/6); INSUFFICIENT_N at annual (n=14). Split-conformal as configured is a *failed uncertainty quantifier* at semiannual cadence.
> - **F3 phase: structural, not a τ artifact (F-4).** Uncapping τ 120→163 cut dense SSE 62% but moved blind annual detrended r only 0.41→0.44 (<0.5); cross-corr lag unchanged at ~25 days. `f3_restored=False` (script 28). F3's seasonal comes from the un-lagged carrier, so no τ re-phases it.
> - **FEASIBILITY (new):** sub-annual per-layer *dynamics* from surface + 1D head are mathematically underdetermined and empirically unrecoverable **at annual/semiannual cadence** — proven by the amplitude-bound (F2 seasonal 4.71 mm > surface 3.83 mm) and carrier rank-1 (all layers ∝ d(t), SVD rank 1) exhibits (script 30). Cadence-specific, not absolute: monthly recovers F3 (r=0.862).
>
> **GATE M9 — portfolio survey persisted.** 29 stations run, 8 excluded (ERLUN no GPS + 7 with GPS–MLCW overlap < 300 epochs); portfolio median holdout RMSE 2.52 mm; GPS-to-well distance does NOT predict accuracy (Pearson r = −0.21, n = 148) — the amplitude/dynamics mismatch, not distance, is the limiter. `m5_deployment/summary/`.

> **Net deployable claim (corrected):** secular trend apportionment (< 1% trend error) + datum maintenance by sparse field visits + partial F2 seasonal dynamics. **NOT** claimed: sub-annual multilayer dynamics reconstruction at sparse cadence.

> **F3 FORENSIC TRIAGE (2026-06-12, scripts 31–33, `results/seq/forensics/`).** Drilled into *why* F3 fails. Verdict: **CONFLUENCE, code exonerated** (`discussions/F3_FORENSIC_VERDICT_20260612.md`).
> - **Code sound (script 31):** synthetic solver recovers an injected τ=200 EXACTLY (S_ke 2.02/2.0, S_kv 4.95/5.0). Lag recovery survives only to driver peak-\|r\| ≈ 0.58; the real F3 well sits at 0.10–0.24 → the lag is unfindable. Driver quality, not the code, governs recovery.
> - **H1 depth-mismatched driver (SUPPORTED, script 32):** F3 well `09050331` is screened 176–179 m but the compacting clay is 238–275 m → **79 m gap, 0 m overlap**; its detrended-seasonal \|r\| with F3 is **0.10** (v4's 0.24 was trend-only; reproduced 0.39 trend-inclusive). Ranks 237/242. Top-correlating wells are 20–55 km distant common-mode (not physical); best LOCAL well only 0.65 at an 810-day lag.
> - **H3 physical outlier (SUPPORTED, Phase 0 NotebookLM):** deep CRAF clay is intrinsically out-of-phase with shallow head (TKSH/Tuku observed 2004–07; soil creep + Noordbergum; surface inversion rank-deficient). Diagnosed F3 lag ~815 d exceeds the literature 90%-compaction bound (488 d) — secondary-compression signature.
> - **H2 poisoned truth (PARTIAL):** F3 truth non-integer fraction 15%→42%→**100% (2024+)**; pre-2019 genuine (rough like F2), recent era smoothed. 2024 F3 grades not field-verifiable.
