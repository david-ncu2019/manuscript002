...
Standardization of multi-source monitoring data constructed a uniform monthly feature matrix ($X \in \mathbb{R}^{N \times P}$) aligned with section-level compaction targets ($y \in \mathbb{R}^{N \times 6}$). 

Subsurface compaction targets were derived by differencing cumulative extensometer ring displacements into monthly increments across six uniform 50~m depth intervals (S1: 0 to 50~m through S6: 250 to 300~m). This sectioning harmonizes variable extensometer anchor depths across stations into consistent depth-dependent compaction series.

Sediment facies compositions (gravel, coarse sand, fine sand, and clay/silt) derived from borehole logs and 3D hydrogeological model extractions were transformed to eliminate constant-sum closure and multicollinearity. An Isometric Log-Ratio (ILR) transformation constructed via a Sequential Binary Partition (SBP) hierarchy mapped the four volumetric percentages into three unconstrained, orthogonal log-ratio balances \citep{egozcue_isometric_2003, oh_using_2024}.

Groundwater level (GWL) series were zero-referenced to baseline epochs and differenced monthly ($\text{dGWL} = H_t - H_{t-1}$). This first-difference transformation removes arbitrary datum offsets and achieves temporal stationarity across seasonal pumping drawdowns and monsoon recovery cycles.

Continuous GNSS position series and 2D vector-decomposed InSAR vertical displacement fields were resampled to monthly steps at the MLCW station coordinates. Spatial extractions from the 3D hydrogeological model provided continuous stratigraphy across unmonitored zones. \Cref{fig:preprocessing_workflow} illustrates the overall spatiotemporal alignment workflow.
