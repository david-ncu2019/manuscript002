...
The 90\% Bayesian posterior predictive intervals contained 78.1\% of the observations, below the nominal 90\% level, indicating the intervals were narrower than required. Possible sources of this gap are considered in \Cref{subsec:discussion_limitations}. Point-estimate error was worst in S5 and S6. Coverage, at 81.2\% in S5 and 65.9\% in S6, was not worst in both of those same sections. It reached its lowest value in S6, while S1 (74.6\%) and S3 (73.9\%) also fell below S5. Reliability therefore did not follow the same depth pattern as point-estimate error.

\begin{table}[htbp]
  \centering
  \small
  \renewcommand{\arraystretch}{1.15}
  \caption[Performance and posterior predictive interval statistics during delayed MLCW delivery.]{Point-estimate errors, model fit, and 90\% Bayesian posterior predictive interval statistics for monthly deformation estimates. Each depth section contains 138 estimates. RMSE, MAE, and Width are reported in mm/month. The row labelled All combines estimates from the six sections.}
  \label{tab:delayed_performance_interval}
  \begin{tabular}{lrrrrr}
    \toprule
    \textbf{Section} & \textbf{$R^2$} & \textbf{RMSE} & \textbf{MAE} & \textbf{Coverage (\%)} & \textbf{Width} \\
    \midrule
    S1 & 0.888 & 0.212 & 0.178 & 74.6 & 0.541 \\
    S2 & 0.976 & 0.239 & 0.182 & 88.4 & 0.637 \\
    S3 & 0.870 & 0.229 & 0.153 & 73.9 & 0.442 \\
    S4 & 0.798 & 0.311 & 0.259 & 84.8 & 0.984 \\
    S5 & 0.077 & 0.661 & 0.503 & 81.2 & 1.847 \\
    S6 & 0.338 & 0.621 & 0.396 & 65.9 & 0.971 \\
    \midrule
    All & 0.778 & 0.423 & 0.279 & 78.1 & 0.904 \\
    \bottomrule
  \end{tabular}
\end{table}

Monthly estimation errors did not increase steadily with time since the preceding MLCW update (\Cref{tab:delayed_performance_by_month}). MAE ranged from 0.251 to 0.336~mm/month across the six positions, with the highest value in month 2 rather than month 6.

\begin{table}[htbp]
  \centering
  \small
  \renewcommand{\arraystretch}{1.15}
  \caption[Performance by month position within each estimation cycle.]{Performance grouped by month position within each six-month estimation cycle. Each monthly position contains 138 estimates. Mean error follows $e=\hat{y}-y$. MAE, RMSE, Mean error, and Width are reported in mm/month.}
  \label{tab:delayed_performance_by_month}
  \begin{tabular}{lrrrrr}
    \toprule
    \textbf{Month} & \textbf{MAE} & \textbf{RMSE} & \textbf{Mean error} & \textbf{Coverage (\%)} & \textbf{Width} \\
    \midrule
    1 & 0.251 & 0.353 & $-0.013$ & 80.4 & 0.909 \\
    2 & 0.336 & 0.592 & $-0.001$ & 73.2 & 0.910 \\
    3 & 0.273 & 0.391 & $-0.001$ & 79.0 & 0.905 \\
    4 & 0.275 & 0.376 & 0.048 & 74.6 & 0.904 \\
    5 & 0.257 & 0.352 & 0.043 & 82.6 & 0.896 \\
    6 & 0.280 & 0.423 & 0.007 & 79.0 & 0.897 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ccrrrrr}
    ...
    \midrule
    6  & 3 & 0.278 & 0.433 & 82.9 & 0.943 & 1.405 \\
    6  & 5 & 0.269 & 0.443 & 89.4 & 1.109 & 1.426 \\
    6  & 8 & 0.299 & 0.448 & 87.3 & 1.352 & 1.542 \\
    12 & 3 & 0.287 & 0.441 & 77.8 & 0.872 & 2.470 \\
    12 & 5 & 0.273 & 0.446 & 86.6 & 1.072 & 2.474 \\
    12 & 8 & 0.308 & 0.454 & 81.9 & 1.317 & 2.738 \\
    \bottomrule
  \end{tabular}
\end{table}
...
Extending the initial record from 3 to 8 years did not reduce error consistently. The 5-year record produced the lowest monthly MAE under both schedules, whereas the 8-year record produced the highest monthly and endpoint MAE. Coverage of the nominal 90\% intervals ranged from 77.8 to 89.4\%, remaining below the nominal level in all six scenarios. As in \Cref{subsec:results_delayed_delivery}, this shortfall reflects the discrepancy between the observed coverage and the nominal interval level rather than a difference in interval width; possible mechanisms are discussed in \Cref{subsec:discussion_limitations}.
...
Coverage of the nominal 90\% intervals ranged from 59.2 to 85.0\% across depth sections, pooled over all three initial-record lengths, remaining below the nominal level in every section (\Cref{tab:results_no_subsequent_mlcw_interval}). As in \Cref{subsec:results_delayed_delivery}, this shortfall reflects the discrepancy between the observed coverage and the nominal interval level rather than a difference in interval width; possible mechanisms are discussed in \Cref{subsec:discussion_limitations}.

\begin{table}[htbp]
  \centering
  \small
  \renewcommand{\arraystretch}{1.2}
  \caption[Monthly error and posterior predictive interval statistics without subsequent MLCW measurements.]{Monthly error and posterior predictive interval statistics without subsequent MLCW measurements, pooled across the three initial-record lengths for each depth section (240 estimates per row) and across all scenarios for the All row (1440 estimates).}
  \label{tab:results_no_subsequent_mlcw_interval}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Section} & \shortstack{\textbf{MAE}\\\textbf{(mm/month)}} & \shortstack{\textbf{RMSE}\\\textbf{(mm/month)}} & \shortstack{\textbf{Coverage}\\\textbf{(\%)}} & \shortstack{\textbf{Width}\\\textbf{(mm/month)}} \\
    \midrule
    S1  & 0.197 & 0.229 & 85.0 & 0.703 \\
    S2  & 0.149 & 0.215 & 74.6 & 0.556 \\
    S3  & 0.138 & 0.276 & 74.2 & 0.421 \\
    S4  & 0.165 & 0.277 & 59.2 & 1.010 \\
    S5  & 0.391 & 0.566 & 76.2 & 1.665 \\
    S6  & 0.356 & 0.693 & 79.2 & 1.281 \\
    All & 0.233 & 0.419 & 74.7 & 0.939 \\
    \bottomrule
  \end{tabular}
\end{table}
