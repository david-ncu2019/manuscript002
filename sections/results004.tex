...
Monthly estimation errors did not increase steadily as each six-month estimation period progressed (\Cref{tab:delayed_performance_by_month}). Mean MAE ranged from 0.25 to 0.34~mm/month, and mean RMSE ranged from 0.31 to 0.50~mm/month. Both reached their highest values in month 2 rather than month 6, while mean coverage varied from 73.2\% to 82.6\% without a consistent temporal pattern. The later months of the estimation period therefore did not show a progressive loss of estimation performance.
...
\begin{table}[htbp]
  ...
  \small
  \renewcommand{\arraystretch}{1.15}
  \caption[Performance by month position within each estimation cycle.]{Performance by position within each six-month estimation cycle (months 1--6, not calendar months). Mean $\pm$ SD across six depth sections; metrics per \Cref{tab:evaluation_metrics}.}
