The methodology followed three stages. First, the monitoring records were converted into monthly response and predictor variables for each depth section. Bayesian ridge regression was then used to estimate monthly compaction. Finally, the estimates and their uncertainty were evaluated during temporary gaps in MLCW data availability and under hypothetical schedules with less frequent MLCW measurements.

\subsection{Preparation of model inputs}
\label{subsec:predictor_preparation}

The observations described in \Cref{subsec:datasets} were first converted to a common monthly time scale. A deformation time series model aligned the MLCW and cGNSS displacement records and produced monthly increments. The sediment proportions were represented by isometric logratio coordinates, and the transformed records were combined with hydraulic head changes and seasonal terms to form the regression dataset.
...
\end{equation}
where $d_0$ denotes displacement at reference time $t_0$, $v$ denotes linear velocity, $a_k$ and $b_k$ describe the periodic components, and $H(t-t_j)$ represents a step at discontinuity time $t_j$ \citep{nikolaidis_observation_2002}. Evaluation at the common monthly dates produced aligned cumulative displacement records. Differences between consecutive fitted values represented the MLCW compaction and cGNSS vertical displacement that occurred during each month. The resulting monthly MLCW increments were used directly as the response variable in the subsequent regression analysis.

\subsubsection{Isometric logratio transformation of sediment composition}
\label{subsec:ilr_transformation}

Sediment composition provided time-independent information that distinguished the six depth sections. Within each section, composition was expressed as the proportions of gravel, coarse sand, fine sand, and fine-grained deposits. Because the four proportions summed to 100\%, an increase in one proportion required a decrease in at least one other proportion. Treating all four proportions as independent predictors would therefore introduce exact linear dependence.

An isometric logratio transformation represented the relative composition with three independent coordinates \citep{egozcue_isometric_2003, oh_using_2024}. A sequential binary partition first divided the four sediment components into physically interpretable groups. Each coordinate expressed the logarithmic ratio between the geometric means of two groups. For groups containing $r$ and $s$ components, the corresponding balance was
\begin{equation}
z=\sqrt{\frac{rs}{r+s}}
...
After temporal alignment and compositional transformation, each observation represented one depth section during one month. The corresponding response was the monthly MLCW compaction increment. The predictors described hydraulic conditions, vertical surface displacement, sediment composition, and seasonal variation.

Monthly hydraulic head changes were included for the target section and for the other depth sections at Tuku. The cGNSS increments represented the concurrent vertical surface displacement, while the sediment balances distinguished lithology among sections. Annual and semiannual terms represented recurring seasonal variation. Current and lagged hydraulic head and cGNSS changes represented immediate and delayed associations with monthly compaction.

Data from all six sections were combined in one station-specific calibration dataset. Section indicators and selected interaction terms allowed the relations with compaction to vary with depth, season, and sediment composition. Before each six-month evaluation block, predictor centering and scaling were calculated only from the calibration observations available at that time and were then applied to the evaluation block. Previous MLCW compaction values were not used as predictors. \Cref{tab:predictor_summary} summarizes the information represented by each predictor group, while \Cref{app:predictors} records the final variable names, temporal transformations, and lag periods.
