...
\end{equation}
%
The GTWR terms mirror those in GWR but add the temporal dimension $t_i$. To apply this framework to our study, we model the compaction within a specific aquifer layer as a function of the corresponding surface deformation derived from SBAS-InSAR. The general GTWR model from \Cref{eq:gtwr} is therefore specified for this research as:
%
\begin{equation}
...
\end{equation}
%
\noindent where the compaction within a specific layer \(n\) at well \(i\), represented by \(\text{MLCW}_{i,n}(t)\), is modeled as a function of the corresponding InSAR-derived surface deformation, denoted by \(\text{InSAR}_i(t)\). The spatiotemporally varying terms \(\beta_0\) and \(\beta_1\) capture the relationship across space and time between these variables, respectively.
%
