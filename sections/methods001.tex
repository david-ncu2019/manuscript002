...
\noindent where \(y_i\) denotes the dependent variable at the \(i\)-th location; \(x_{ik}\) represents the independent variables (or predictors), with \(x_{i0} = 1\) at the intercept \(\beta_0\); \(\beta_k\) indicates the regression coefficient corresponding to the \(k\)-th predictor; \(\varepsilon_i\) is the random error at the \(i\)-th point, and \(p\) is the number of independent variables (excluding the intercept).
%
The OLS assigns a single set of coefficients to all observations regardless of their geographical locations, which deliberately neglects potential spatial variation and might cause model misspecification \citet{mgwr_book_2017}.
%
% ========================================
% Section: Recap GWR concept
% ========================================
%
The GWR approach allows the relationships to vary spatially, 
Prior to discussing the GTWR model, it is useful to recap the fundamental concept of the geographically weighted regression (GWR) approach \citep{brunsdon1996, fotheringham2002geographically}, defined as:
