...
Reduced MLCW measurement frequency was examined under hypothetical schedules in which MLCW observations became available only at the end of each measurement interval, while monthly GWL and cGNSS observations remained available throughout to estimate monthly deformation increments. Intervening monthly MLCW values were excluded from estimation and model updating, with each interval instead contributing one cumulative MLCW deformation observation at its endpoint. The evaluated scenarios combined measurement intervals of 6 or 12 months with initial calibration periods containing 3, 5, or 8 years of consecutive monthly MLCW observations, all ending on the same calendar date, so the 3- and 5-year periods began later than the 8-year period rather than starting from the same date and ending earlier. This shared calibration endpoint let the three initial-history lengths be compared over an identical subsequent period, undergoing the same number of model updates at the same calendar dates within a given measurement schedule. Because a 6-month schedule and a 12-month schedule complete different numbers of measurement intervals over the same period, the two schedules were compared using their own respective update counts, with matching update counts and dates required only for the initial-history-length comparison. These durations defined the scenarios examined in this study rather than a requirement of the method.
