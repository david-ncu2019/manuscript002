...
Each row in the regression dataset represented one depth section during one month, one observation. The observed deformation increment $\Delta d_{s,k}$ for that section-month served as the response, the quantity the model was trained to estimate. The predictors, the input variables used to estimate that response, described hydraulic conditions, vertical surface displacement, and seasonal variation.
...
The Bayesian formulation provided both a monthly deformation estimate and a measure of uncertainty around that estimate. This uncertainty was represented by the posterior predictive distribution, which combines variation not explained by the regression model with uncertainty in the fitted regression coefficients \citep{mackay_bayesian_1992}. For a new month with standardized predictors $\boldsymbol{x}_{s,*}$ and calibration data $\mathcal{D}_s$, the posterior predictive distribution for section $s$ was
