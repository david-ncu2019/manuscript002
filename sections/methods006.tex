...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.21\textwidth}p{0.27\textwidth}X@{}}
    ...
    Hydraulic head in the target section & Current change, lagged values, means over three and six months, second difference, and absolute value & Hydraulic conditions in the section being estimated \\
    Hydraulic head across the monitored profile & Current change and means over three and six months for all six sections & Hydraulic conditions throughout the monitored depths \\
...
Interval performance was evaluated using empirical coverage and mean interval width, with \Cref{tab:evaluation_metrics} giving the notation, definition, and unit for every metric based on point estimates or intervals reported in \Cref{sec:results}. Empirical coverage measured the proportion of observed monthly increments enclosed by their corresponding intervals. Its agreement with the nominal 90\% level indicated how closely the intervals represented the uncertainty encountered during evaluation. Mean interval width described the range of plausible monthly increments around each estimate. Coverage and width were interpreted together because a narrower interval provided more precise information only when it continued to enclose the expected proportion of observations \citep{gneiting_probabilistic_2007, singh_uncertainty_2024}.

\begin{table}[!ht]
  \centering
  \small
  \renewcommand{\arraystretch}{1.3}
  \caption[Evaluation metrics.]{Evaluation metrics used to summarize monthly and endpoint estimation performance. The monthly residual was $e_{s,k}=\widehat{\Delta d}_{s,k}-\Delta d_{s,k}$, the estimated minus the observed monthly deformation increment for section $s$ and month $k$, and $\overline{\Delta d_{s}}$ is the mean observed monthly increment for section $s$; this same residual convention is used again in \Cref{eq:sparse_monthly_error}. Each metric was calculated over the estimates for the sections and months, or the sections and endpoints, stated in the corresponding table.}
  ...
  \begin{tabular}{lp{0.24\textwidth}p{0.4\textwidth}l}
    ...
    Coverage & Prediction interval coverage probability & Percentage of observations enclosed by the nominal 90\% posterior predictive interval & \% \\
    Width & Mean prediction interval width & $\overline{W}=\dfrac{1}{n}\sum 2\left(1.645\,\sigma_{s,k}\right)$ & mm/month \\
...
Point estimates were evaluated separately for each depth section using $R^2$, RMSE, and MAE. Posterior predictive intervals were evaluated using empirical coverage and mean interval width, as described in \Cref{subsec:predictive_uncertainty}. Performance was also compared across successive months within each update cycle to determine whether estimation quality changed as the MLCW observation gap lengthened.
...
\begin{figure}[htbp]
  ...
  \caption{Estimation and model updating during the temporary unavailability of finalized monthly deformation records for individual depth intervals. Hydraulic head and vertical surface displacement remained available during this period. Filled blue markers denote available observations, open blue markers denote deformation records not yet available, and orange markers denote the records subsequently provided. The symbol $\widehat{\Delta d}_{s,k}$ denotes the estimated monthly deformation increment for section $s$. Dots indicate omitted records; marker counts and spacing are illustrative.}
...
Monthly errors described how well the model distributed deformation within each interval, whereas endpoint errors described agreement with the cumulative field observation. Both error types were summarized using MAE and RMSE for each depth section. The coefficient of determination was calculated for the monthly estimates but not for the endpoint observations, because each scenario contained relatively few cumulative measurements. Posterior predictive intervals for the monthly estimates were evaluated using empirical coverage and mean interval width.

The summary errors alone did not show whether differences among the monitoring scenarios persisted across measurement cycles. Two additional comparisons therefore used the same 72 calendar months and the same six depth sections. The first compared each scenario with less frequent measurements against estimation with delayed monthly records, while the second compared initial records of 3, 5, and 8 years within each measurement schedule. Matching the evaluated months and depth sections prevented differences in the available observations from being mistaken for effects of measurement frequency or the length of the initial record.

Monthly errors within one measurement cycle were produced before the model was updated again. Complete cycles were therefore selected together when assessing the stability of each MAE difference, rather than treating errors for individual sections and months as independent observations. Repeated selection of these cycles produced a 95\% resampling interval for each difference. The two comparisons, their subtraction order, and the detailed resampling procedure are described in \Cref{app:paired_mae_comparisons}.
...
Because the three calibration records shared the same endpoint, all three left the same 80 months for retrospective evaluation, over the identical subsequent calendar period. The complete evaluation, in which each model was fitted once, is illustrated in \Cref{fig:no_subsequent_mlcw_design}.
...
\begin{figure}[htbp]
  ...
  \caption{Monthly deformation estimation under the hypothetical absence of subsequent deformation measurements for individual depth intervals. Filled blue markers denote observations used to fit each model once. Hydraulic head and vertical surface displacement remained available, while open gray markers denote monthly estimates $\widehat{\Delta d}_{s,k}$. Model coefficients and predictor scaling remained fixed throughout the estimation period. Dots indicate omitted records; marker counts and spacing are illustrative.}
